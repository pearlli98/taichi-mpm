...
std::string MPM<dim>::add_particles(const Config &config) {
  ...
  if (config.get_string("type") == "rigid") {
    ...
    auto create_particle = [&](const Vector &coord, real maximum,
      ...
      p->pos = coord;
      // std::cout << "get coordinate" << count << coord << "\n";
...
std::vector<RenderParticle> MPM<dim>::get_render_particles() const {
  std::cout << "------ get render particles printed ----";
  // for (uint32 i = 0; i < particles.size(); i++) {
  //   std::cout << allocator[particles[i]]->pos;
  // }
...
void MPM<dim>::step(real dt) {
  ...
  std::cout << request_t << "\n";
  ofstream myfile;
  string name = "output" + to_string(int(request_t * 100)) + ".txt";
  myfile.open(name);
  for (uint32 i = 0; i < particles.size(); i++) {
    ParticlePtr p_i = particles[i];
    auto p = allocator[p_i];
    myfile << p_i;
    myfile << p->pos << p->get_velocity() << "\n";
  }
  myfile.close();
